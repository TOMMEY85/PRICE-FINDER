...
const SSD_NVME_2TB_CATEGORY="https://www.materiel.net/disque-ssd/l429/%2Bfc2456-1%2Bfi62-l2048h2048/";
...
async function verifyProductPage(url,q){
  try{
    ...
    const products=productsFromJsonLd(html,url,"Materiel.net").filter(p=>matchesProductQuery(p,q));
...
export async function searchMateriel(q){
  ...
  else if(isStorageQuery(q) && /\b2\s*(to|tb)\b/i.test(q) && /\bnvme\b/i.test(q)) listingUrl=SSD_NVME_2TB_CATEGORY;
  else listingUrl=`https://www.materiel.net/recherche/${encodeURIComponent(q)}.html`;

  const html=await fetchHtml(listingUrl,{timeout:12000});
  const listingJson=productsFromJsonLd(html,BASE,"Materiel.net").filter(p=>matchesProductQuery(p,q));
